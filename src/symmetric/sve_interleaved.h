...
#if defined(CAMBLAS_MICRO8_NEON12_B8) && CAMBLAS_MICRO8_NEON12_B8
#include "neon12_b8.h"
#endif
...
static inline size_t camblas_micro8_a_index(size_t row, size_t depth, size_t ld, size_t group,
                                            int micro)
{
    /* Private layout 2 pads a three-vector A group to four vectors. Callers
     * must allocate the physical stride and use the matching vector length. */
    const size_t physical = micro == 2 ? group / 3 * 4 : group;
    return micro ? (row / group * physical) * ld + depth * physical + row % group
                 : row + depth * ld;
}
static inline int camblas_micro8_layout_f32_base(int m, int n, int k, float alpha, const float *A,
...
{
#if defined(CAMBLAS_MICRO8_NEON12_B8) && CAMBLAS_MICRO8_NEON12_B8
    if (!a_micro && m >= 8 && n >= 24 && k > 0 && svcntb() == 16) {
        int rows = m / 8 * 8, cols = n / 24 * 24;
        for (int j = 0; j < cols; j += 12)
            for (int i = 0; i < rows; i += 8)
                camblas_neon12_b8_f32(k, alpha, A + i, lda, B, ldb, j, C + i + (size_t)j * ldc, ldc,
                                      initialize);
        /* Multiples of 24 keep the residual B panel eight-column aligned.
         * The original SVE body retains all predicated-edge contracts. */
        if (rows < m)
            camblas_micro8_layout_f32_base(m - rows, n, k, alpha, A + rows, lda, B, ldb, C + rows,
                                           ldc, 0, initialize);
        if (cols < n)
            camblas_micro8_layout_f32_base(rows, n - cols, k, alpha, A, lda, B + (size_t)cols * ldb,
                                           ldb, C + (size_t)cols * ldc, ldc, 0, initialize);
        return 0;
    }
#endif
    return camblas_micro8_layout_f32_base(m, n, k, alpha, A, lda, B, ldb, C, ldc, a_micro,
                                          initialize);
}
