...
static void rectangular64_transpose_two(const double *left, const double *right, int sign,
                                        size_t stride, double *destination)
{
    float64x2_t a = rectangular64_load_sum(left, right, sign);
    float64x2_t b = rectangular64_load_sum(left + stride, right ? right + stride : NULL, sign);
...
static void rectangular64_pack_all_b(rectangular64_t *w, int column, int pc, int bk)
{
    int count = w->hn - column < 8 ? w->hn - column : 8;
    ...
    for (int index = total * worker / w->workers; index < total * (worker + 1) / w->workers;
        ...
        if (group < a_blocks * weight) {
            ...
            if (last > w->row_groups)
                last = w->row_groups;
            rectangular64_pack_all_a(w, first, last, pc, bk);
        } else {
            group -= a_blocks * weight;
            if (!w->tb)
                rectangular64_pack_b_block(w, group / w->column_groups,
                                           group % w->column_groups * 8, pc, bk);
            else if (group % 7 == 0)
                rectangular64_pack_all_b(w, group / 7 * 8, pc, bk);
        }
    }
}
                                              ...
                                              void *scratch, size_t bytes)
{
    ...
    if (executor->run(rectangular64_pack, workers_tasks, workers, &work, executor->user_data) ||
        executor->run(rectangular64_product, product_tasks, count, &work, executor->user_data) ||
