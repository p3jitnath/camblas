/* Full-tile candidates with all four A vectors live and diagonal FMA order.
 * This trades accumulator count for input-register diversity.
 */
#ifndef CAMBLAS_SVE_BLOCK_BALANCED_H
#define CAMBLAS_SVE_BLOCK_BALANCED_H
#include "sve_lane_ops.h"
#ifndef CAMBLAS_SVE_BALANCED_COLS
#define CAMBLAS_SVE_BALANCED_COLS 0
#endif
#if CAMBLAS_SVE_BALANCED_COLS != 0 && CAMBLAS_SVE_BALANCED_COLS != 4 && \
    CAMBLAS_SVE_BALANCED_COLS != 5
#error "Balanced column count must be 0, 4 or 5"
#endif
#if CAMBLAS_SVE_BALANCED_COLS == 4
static inline __attribute__((always_inline)) void camblas_balanced_f32(int k, float alpha,
...
static inline __attribute__((always_inline)) void camblas_balanced_f64(int k, double alpha,
...
#endif
#if CAMBLAS_SVE_BALANCED_COLS == 5
static inline __attribute__((always_inline)) void camblas_balanced_f32(int k, float alpha,
                                                                       const float *A, size_t lda,
                                                                       const float *B, size_t ldb,
                                                                       float *C, size_t ldc)
