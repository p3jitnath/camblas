/* FP32 twelve-row A-micro kernel in a separate translation unit so that its
...
#endif
int camblas_sgemm_sve_amicro12_tile(int m, int n, int k, float alpha, const float *A, int lda,
                                    const float *B, int ldb, float *C, int ldc, int initialize)
{
    ...
    /* Three four-lane FP32 vectors cover twelve rows. Refuse other runtime
